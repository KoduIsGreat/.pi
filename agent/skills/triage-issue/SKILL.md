...
Dispatch a `scout` subagent to deeply investigate the codebase. Your goal is to find:
...
subagent: {
  agent: "scout",
  task: "Thorough recon for bug: <user's description>. Return: (1) where the bug manifests (entry points, UI, API), (2) the code path involved (traced end-to-end), (3) the root cause (not just symptom), (4) existing tests covering this area and any gaps, (5) recent git log on affected files, (6) similar patterns elsewhere that work correctly. Do NOT propose fixes."
}
```
