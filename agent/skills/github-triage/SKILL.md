...
- Dispatch a `scout` subagent to build codebase context — understand the domain, relevant interfaces, and existing behavior related to the issue:

  ```
  subagent: {
    agent: "scout",
    task: "Recon for GitHub issue #<num>: <one-line summary>. Return: (1) relevant modules/files, (2) domain terms (check UBIQUITOUS_LANGUAGE.md/CONTEXT.md if present), (3) existing behavior in this area, (4) anything that looks like a reproduction path. Do NOT propose fixes."
  }
  ```
